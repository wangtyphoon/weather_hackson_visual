...
function setupstackbarmap(data, specificIndustry) {
  ...
  function click() {
    ...
    let result = result2020.concat(result2021)
    console.log(result)
    draw_stackbar(result, title,target); // 绘制图表
  }
    ...
    if (target == "碳排放"){
      let result = data.filter(item => item['行業'] === specificIndustry) // 根据特定行业参数筛选数据
        .map(item => ({
          ...
          '公司': item['公司'],
          '年分': year,
          '範疇一': Math.round(Math.abs(Number(item[year + '範疇一'].replace(/,/g, ''))) ), // 将特定属性值进行数值处理
          '範疇二': Math.round(Math.abs(Number(item[year + '範疇二'].replace(/,/g, ''))) ),
          '範疇三': Math.round(Math.abs(Number(item[year + '範疇三'].replace(/,/g, ''))) )
        }));
        return result;
      }
    if (target == "資源消耗"){
      let result = data.filter(item => item['行業'] === specificIndustry) // 根据特定行业参数筛选数据
        .map(item => ({
          ...
          '公司': item['公司'],
          '年分': year,
          '能源使用': Math.round(Math.abs(Number(item[year + '能源使用'].replace(/,/g, '')))/1000 ), // 将特定属性值进行数值处理
...
function draw_stackbar(res, title,target) {
  ...
  else if(target == '廢棄物製造'){
    project = ["廢棄物總量","有害廢棄物"]
  }
  
  const width = document.body.clientWidth*0.5; // 设置画布宽度为 1000 像素
  const height = 720; // 设置画布高度为 600 像素
  const chart_margin = { top: 80, right: 80, bottom: 40, left: 80 }; // 设置图表的边距，包含上、右、下、左四个方向的边距值
